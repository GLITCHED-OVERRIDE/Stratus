	...
	globalData: [function(a, t, e) {
		...
		cc._RF.push(t, "0d035p4mEFLZp6TvkrDvVq8", "globalData"), window.globalData = {
			...
			setGunLevel: function(a) {
				this.data.gunLevel[a]++, this.data.gunLevel[a] >= 5 && globalData.setHuoDongData("é‡‘æžªå¤§å¸ˆ"), this.saveData()
			},
			...
			setRoleUnlockState: function(a) {
				...
				this.data.roleUnlockState[a] = 2, globalData.roleType = a, a >= 12 && cc.director.emit("è§£é”å½©è›‹è§’è‰²", a), cc.director.emit("åˆ‡æ¢å±•ç¤ºè§’è‰²", a), this.saveData()
			},
			...
			getQiangHua: function(a) {
				...
				if (null != t) return t;
				console.error("å¼ºåŒ–å±žæ€§èŽ·å–å¤±è´¥  " + a)
			},
			setQiangHua: function(a, t) {
				...
				return 0 == e && console.error("å‚æ•°é”™è¯¯  setQiangHua"), this.saveData(), cc.director.emit("ç³»ç»Ÿæç¤º", a + " Update Successed"), !0
			},
			...
			setQiangHuaLevel: function() {
				return (arguments.length <= 0 ? void 0 : arguments[0]) ? (this.data.qiangHuaLevel = 0, this.data.qiangHuaNum = this.data.qiangHuaNumForever, setTimeout(function() {
					cc.director.emit("ç³»ç»Ÿæç¤º", "Skill Points Reset")
				}, 300)) : this.data.qiangHuaLevel += 1, cc.director.emit("å¼ºåŒ–ç­‰çº§å˜åŒ–"), cc.director.emit("å¼ºåŒ–ç‚¹æ•°é‡å˜åŒ–"), this.saveData(), !0
			},
			getQiangHuaNum: function() {
				return this.data.qiangHuaNum
			},
			setQiangHuaNum: function(a) {
				return this.data.qiangHuaNum + a >= 0 ? (this.data.qiangHuaNum += a, a > 0 && (this.data.qiangHuaNumForever += a), console.log("å½“å‰å¼ºåŒ–ç‚¹  " + this.data.qiangHuaNum), cc.director.emit("å¼ºåŒ–ç‚¹æ•°é‡å˜åŒ–"), this.saveData(), !0) : (cc.director.emit("ç³»ç»Ÿæç¤º", "Need Points"), !1)
			},
			getCoinNum: function() {
				return this.data.coinNum
			},
			setCoinNum: function(a) {
				return this.data.coinNum + a >= 0 && (this.data.coinNum += a, cc.director.emit("é‡‘å¸æ•°é‡å˜åŒ–"), this.saveData(), !0)
			},
			getDiamondNum: function() {
				return this.data.diamondNum
			},
			setDiamondNum: function(a) {
				return this.data.diamondNum + a >= 0 && (this.data.diamondNum += a, cc.director.emit("é’»çŸ³æ•°é‡å˜åŒ–"), this.saveData(), !0)
			},
			getDataAll: function() {
				this.clearDataNow && globalData.clearAllData(), cc.debug.setDisplayStats(!1), 1 != cc.sys.localStorage.getItem(globalData.keyFirst) ? (cc.sys.localStorage.setItem(globalData.keyFirst, 1), globalData.saveData(), cc.log("é¦–æ¬¡è¿›å…¥æ¸¸æˆ"), globalData.data = globalData.getData()) : (cc.log("éžé¦–æ¬¡è¿›å…¥æ¸¸æˆ " + cc.sys.localStorage.getItem(globalData.keyFirst)), globalData.data = globalData.getData());
